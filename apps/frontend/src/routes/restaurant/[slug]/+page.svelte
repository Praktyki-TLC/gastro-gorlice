<script lang="ts">
  ...
  const now = new Date();
  const calendarDate = $derived((lastMenuDate > now ? now : lastMenuDate).toISOString().split("T")[0]);
  ...
  let calendarRef: HTMLElement & { value: string };
      ...
      datasets: [
        {
          ...
          borderColor: isDay ? "oklch(42% 0.199 265.638)" : "oklch(43% 0.095 166.913)",
          backgroundColor: isDay ? "rgba(59, 130, 246, 0.2)" : "rgba(16, 185, 129, 0.2)",
          tension: 0.3,
          pointRadius: 4,
          pointHoverRadius: 6,
        },
      ...
      <div class="flex-none w-full md:w-auto">
        ...
        >
          <calendar-date
            ...
            today="1970-01-01"
          >
            <svg slot="previous" class="fill-current size-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M15.75 19.5 8.25 12l7.5-7.5" /></svg>
            <svg slot="next" class="fill-current size-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="m8.25 4.5 7.5 7.5-7.5 7.5" /></svg>
    ...
    <section>
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
        ...
        <div role="tablist" class="tabs tabs-boxed">
          <button role="tab" class="tab {chartView === 'day' ? 'tab-active' : ''}" onclick={() => (chartView = "day")}>Dzienny</button>
          <button role="tab" class="tab {chartView === 'week' ? 'tab-active' : ''}" onclick={() => (chartView = "week")}>Tygodniowy</button>
        </div>
      </div>
